...
void	delete_key(char **line, struct s_cursor *cursor, int nbr)
{
	...
	tputs(tgoto(tgetstr("ch", NULL), 0, cursor->co), 1, ft_putchar_err);
...
void	end_of_life(char **line, struct s_cursor cursor, char **env)
{
	int		pos;

	if ((int)ft_strlen(*line) == 0 && env != NULL)
		exit_shell(env);
	pos = line_index(*line, cursor);
...
{
	tputs(tgoto(tgetstr("ch", NULL), 0, cursor.prompt), 1, ft_putchar_err);
	if (cursor.li > 1)
		tputs(tgoto(tgetstr("UP", NULL), 0, cursor.li - 1), 1, ft_putchar_err);
	tputs(tgetstr("cd", NULL), 1, ft_putchar_err);
}
