...
static	void	putstr_select_mode(char *str, int begin, int end)
{
	...
	{
		if (i == begin)
			tputs("\e[46m", 1, ft_putchar_err);
		if (i == end)
			tputs("\e[0m", 1, ft_putchar_err);
		ft_putchar(str[i]);
		i++;
	}
	if (end == i)
	...
	if (index + 1 < len)
	{
		...
		if (pos >= 0)
		{
			*select = ft_strjoin(*select, (char[2]){line[index], '\0'});
			putstr_select_mode(line, index - (int)ft_strlen(*select) + 1, index + 1);
		}
		else
		{
			delete_char(*select, (int)ft_strlen(*select) - 1);
			putstr_select_mode(line, index, index + (int)ft_strlen(*select));
		}
