...
static	void	display_line(t_file *file, int cl, int li)
{
	...
	{
		if (file->nav & 1)
			tputs(tgetstr("so", NULL), 0, ft_putchar_err);
		ft_putstr(file->name);
		if (file->nav & 1)
			tputs(tgetstr("se", NULL), 0, ft_putchar_err);
		space = str_space(cl - (int)ft_strlen(file->name));
		ft_putstr(space);
		i = -1;
		while (file && ++i < li)
			file = file->next;
		...
		int column_size, int nbrli)
{
	...
	quest = ft_strjoir(quest, ft_itoa(complete.len_files), 1);
	quest = ft_strjoir(quest, " possibilities (", 1);
	quest = ft_strjoir(quest, ft_itoa(nbrli), 1);
	quest = ft_strjoir(quest, " lines)? ", 1);
	ft_putstr(quest);
