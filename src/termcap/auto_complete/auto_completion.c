...
static	int			cursor_autocomplete(struct s_termcap info)
{
	...
	while (li <= last)
	{
		tputs(tgetstr("do", NULL), 1, ft_putchar_err);
		li++;
	}
	tputs(tgoto(tgetstr("ch", NULL), 0, 0), 1, ft_putchar_err);
	...
	if (li > 0)
		tputs(tgoto(tgetstr("UP", NULL), 0, li + 1), 1, ft_putchar_err);
